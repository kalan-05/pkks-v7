...
?>
    <section class="pkks-admin-dashboard-intro"><div><p class="pkks-admin-eyebrow">Доступ</p><h2>Пользователи и доступ</h2><p>Здесь можно подготовить приглашение или отключить технический доступ.</p></div><div class="pkks-admin-dashboard-actions"><a class="pkks-admin-button pkks-admin-button--secondary" href="/admin/index.php">Назад</a></div></section>
    ...
    <?php foreach ($users as $user): ?>
        ...
        <?php if ($user['role'] === 'technical_admin'): ?><form class="pkks-admin-login-form" action="/admin/users.php" method="post"><?php echo pkks_admin_csrf_field(); ?><input type="hidden" name="user_id" value="<?php echo (int)$user['id']; ?>"><button name="action" value="resend" type="submit">Новое приглашение</button><button name="action" value="<?php echo (int)$user['active'] === 1 ? 'disable' : 'enable'; ?>" type="submit"><?php echo (int)$user['active'] === 1 ? 'Отключить доступ' : 'Включить доступ'; ?></button></form><?php endif; ?></article>
    <?php endforeach; ?>
    </section>
